...
public class PostStudentByClazzTest {
    ...
    @Test
    public void PostStudentByClazzEndpoint_WithoutPayload_ReturnsSuccess() throws Exception {
        // Arrange
        ...
        // Act
        ResultActions result = mockMvc.perform(post("/api/classes/3A-189/students/00001"));

        // Assert
        result.andExpect(status().isOk())
            .andExpect(jsonPath("$.*", hasSize(2)))
            .andExpect(jsonPath("$.status", is("success")))
            .andExpect(jsonPath("$.message", is("Student with ID [00001] was assigned to Class with ID [3A-189].")));
    ...
    @Test
    public void PostStudentByClazzEndpoint_WithInvalidStudentId_ReturnsNotFoundError() throws Exception {
        // Arrange
        var clazz = buildClazz("3A-189", "Geology", "Sedimentary Petrology");
    ...
    @Test
    public void PutStudentByClazzEndpoint_WithInvalidClazzCode_ReturnsNotFoundError() throws Exception {
        // Arrange
        var student = buildStudent("00001", "Jose", "Perez");
        when(studentRepository.findById("00001")).thenReturn(Optional.of(student));
