...
function ClazzList() {
  ...
  useEffect(() => {
    ...
    let data = newPage<IClazz>();
    getClazzes(page, api.size, sortBy)
      .then(clazzPage => data = clazzPage)
      .finally(() => setRequest({ isLoading: false, data }));
  }, [page, sortBy]);

  const handleCreate = () => authorized(() => navigate('/classes/create'));
    ...
    <>
      <section className="flex flex-col sm:flex-row justify-between items-center gap-2">
        ...
        <button
          ...
          className="flex gap-3 items-center px-5 py-2.5 text-sm text-white bg-gradient-to-r from-sky-500 to-sky-700 hover:bg-gradient-to-br focus:outline-none rounded-full"
        >
